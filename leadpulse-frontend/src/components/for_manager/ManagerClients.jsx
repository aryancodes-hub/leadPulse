...
export default function ManagerClients() {
  const [clients, setClients] = useState([
    { id: "CL-101", name: "Northwind Retail", activeCampaigns: 4, contactEmail: "contact@northwind.com", status: "Active" },
    { id: "CL-102", name: "Brightline Health", activeCampaigns: 2, contactEmail: "admin@brightline.com", status: "Active" },
    { id: "CL-103", name: "Ardent Legal", activeCampaigns: 1, contactEmail: "legal@ardent.com", status: "Paused" },
  ]);
  ...
  const handleCreateClient = async (e) => {
    e.preventDefault();
    const payload = { name: newClientName, contactEmail: newClientEmail };
    try {
      const res = await api.post("/clients", payload);
      const created = res.data?.data ?? res.data;
      setClients([{
        id: created?.id || `CL-${Math.floor(100 + Math.random() * 900)}`,
        name: newClientName,
        activeCampaigns: 0,
      ...
      setClients([{ id: `CL-${Date.now()}`, name: newClientName, activeCampaigns: 0, contactEmail: newClientEmail, status: "Active" }, ...clients]);
    }
    setNewClientName("");
    setNewClientEmail("");
  ...
  const handleToggleStatus = async (clientId, currentStatus) => {
    ...
    try {
      await api.patch(`/clients/${clientId}`, { status: nextStatus });
