...
export default function CampaignInsightsModal({
  ...
  const isCall =
    campaign.type === "Cold Call Blitz" || campaign.type === "call";
  const total = details.length;

  // ── Call Metrics ────────────────────────────────────────────
  const converted = details.filter(
    (c) => c.callOutcome?.toLowerCase() === "converted"
  ).length;
  const conversionRate = total > 0 ? Math.round((converted / total) * 100) : 0;
  const totalTalkTime = details.reduce(
    (acc, c) => acc + (Number(c.callDurationMinutes) || 0),
    0
  );
  ...
  const callbackCalls = details.filter((c) => {
    ...
    return (
      oc.includes("callback") ||
      oc.includes("follow") ||
      oc.includes("scheduled")
    );
  }).length;
  ...
  const notReachedCalls = details.filter((c) => {
    const oc = (c.callOutcome || "").toLowerCase();
    return (
      oc.includes("busy") || oc.includes("voicemail") || oc.includes("no_answer")
    );
  }).length;
  ...
  const rejectedCalls = details.filter((c) => {
    const oc = (c.callOutcome || "").toLowerCase();
    return (
      oc.includes("rejected") || oc.includes("declined") || oc.includes("wrong")
    );
  }).length;

  // ── Email Metrics ────────────────────────────────────────────
  const totalOpens = details.reduce(
    (acc, c) => acc + (Number(c.opens) || 0),
    0
  );
  const totalClicks = details.reduce(
    (acc, c) => acc + (Number(c.clicks) || 0),
    0
  );
  const openRate =
    total > 0 ? ((totalOpens / total) * 100).toFixed(1) : "0.0";
  const clickRate =
    total > 0 ? ((totalClicks / total) * 100).toFixed(1) : "0.0";
  const clickToOpen =
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              >
                ...
                <span className="lp-insights-campaign-id">
                    ...
                    style={{
                      ...
                      borderColor:
                        campaign.status === "Active"
                          ...
                          : "rgba(251,191,36,0.5)",
                      color:
                        campaign.status === "Active" ? "#6ee7b7" : "#fde68a",
                    }}
                  >
                    <span
                      style={{
                        ...
                        borderRadius: "50%",
                        background:
                          campaign.status === "Active" ? "#34d399" : "#fbbf24",
                      }}
                      className={
                        campaign.status === "Active" ? "lp-pulse" : ""
                      }
                    />
                    {campaign.status}
                  </span>
      ...
      <div className="lp-tab-nav">
        {[
          ...
          >
            ...
            {tab.label}
            {tab.key === "log" && (
              <span className="lp-tab-count">{total}</span>
            )}
          </button>
        ))}
      </div>
